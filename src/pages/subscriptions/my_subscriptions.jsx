...
const id = user ? JSON.parse(user).company_id : null;  const navigate = useNavigate();
  ...
  <div className="w-full bg-white border border-gray-200 rounded-lg p-4 mt-6">
    <div className="flex items-center justify-between mb-4">
      <InfoItem
        ...
        value={
          subscription.plan_data?.price == 0
            ? t('trail_subscription')
            : `${t('subscription')}  ${subscription.plan_data?.duration_in_months} ${t('months')}`
        }
      />
      <InfoItem
