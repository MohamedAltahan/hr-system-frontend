...
import {   useGetAllLeavesDataQuery, useUpdate_LeavesRequestMutation } from "../../api/Attendce";
import { toast } from "react-toastify";

const Departuremanagement = () => {
  const { data,  isLoading,refetch } = useGetAllLeavesDataQuery();
  ...
  const [updateLeavesRequest,{isLoading:loading}] = useUpdate_LeavesRequestMutation();
  ...
  const handleUpdateStatus = async (id, status) => {
    ...
    try {
  const response=     await updateLeavesRequest({ id, formData }).unwrap();
          if(response.status==200) {
              toast.success("تم تحديث حالة طلبك بنجاح")
                            }
    } catch (error) {
      toast.error("لقد حدث مشكله اثناء الطلب ")
  ...
  const filteredData = data?.data?.filter((item) => {
    return (
      ...
      (employeeId === "" || item.employee.id.toString() === employeeId) &&
...
<image id="image0_6599_10964" width="64" height="64" xlinkHref="data:image/png;base64,"/>
         ...
         >
           ...
           <tr className="opacity-50 text-[20px]" >
               ...
               <th className="font-400" style={{ padding: '5px' }}> التاريخ من</th>
               <th className="font-400" style={{ padding: '5px' }}> التاريخ الي</th>
               {/* <th className="font-400" style={{ padding: '5px' }}>الوقت</th> */}
               <th className="font-400" style={{ padding: '5px' }}>نوع الطلب      </th>
               <th className="font-400" style={{ padding: '5px' }}>حالة الطلب      </th>
               <th className="font-400" style={{ padding: '5px' }}>السبب      </th>
               <th className="font-400" style={{ padding: '5px' }}>إجراءات     </th>
             </tr>
           </thead>
           <tbody>
             {filteredData.map((item, index) => (
               <tr
                 key={index}
                 className="text-[16px] tracking-wide text-black   border-[#DBDBDB] border-b"
                 style={{
                   height: "60px", // Adjust row height for spacing
                 }}
               >
                 <td style={{ padding: '5px' }}>{item?.employee?.name}</td>
                 <td style={{ padding: '5px' }}>{item?.from_date}</td>
                 <td style={{ padding: '5px' }}>{item?.to_date}</td>
                 <td style={{ padding: '5px' }}>{item?.leave_type=="Vacation"?"مصيف":"مرضي"}</td>
                 <td
                 className="inl"
   style={{
     padding: '5px',
 ...
 >
   {item?.status === "accepted"
     ...
     <p className="bg-[#C8FBCB] text-[#62B85A] p-2 rounded-lg w-[70%] mx-auto">مقبول</p>
     : item?.status === "Pending"
     ? 
     <p className="bg-[#D3D3D3] text-[#919191] p-2 rounded-lg w-[70%] mx-auto">قيد الانتظار</p>
     : 
     <p className="bg-red-200 text-red-500 p-2 rounded-lg w-[70%] mx-auto">تم الرفض</p>
     }
 </td>
   ...
   {item?.status === "accepted" ? (
     <span className="text-[#16C098] font-700 text-xl">تم الموافقة</span>
   ) : item?.status === "Pending" ? (
     <div className="flex flex-row gap-3 justify-center">
       <button                       onClick={() => handleUpdateStatus(item.id, 'accepted')}
  className="text-[#16C098] font-700 text-xl">قبول</button>
       <button   onClick={() => handleUpdateStatus(item.id, 'rejected')} className="text-[#fbb9b9] font-700 text-xl">رفض</button>
     </div>
   ) : (
     <span className="text-[#ef8484] font-700 text-xl">تم الرفض</span>
   )}
 </td>
